...
class CardDisplays:
    ...
    def __init__(self, user: models.user.User,
        ...
        self._sort_method: typing.Optional[
            display_filters.CardDisplaySort] = None
        self._ownership: typing.Optional[
            display_filters.OwnershipFilter] = None
    ...
    def get_value_info(self, all_cards: models.card.AllCards):
        ...
        card_set = np.vectorize(models.card_set.CardSet)(
            card_classes['set_num'])  # class construction might not vectorize

        card_classes['findability'] = get_findability(card_classes['rarity'],
                                                      card_set)
        ...
        values = user_owns_card.create_is_owned_column(values,
                                                       self.user)
        return values
    ...
    def get_page(self, page_num: int = 0) -> pd.DataFrame:
        ...
        card_display_page_generator = CardDisplayPage(self.value_info,
                                                      page_num=page_num)
    ...
    def get_card(self, card_id: models.card.CardId) -> pd.DataFrame:
        ...
        displays_df = self.value_info[
            np.logical_and(self.value_info['set_num'] == card_id.set_num,
    ...
    def configure(self,
                  ...
                  ownership: typing.Type[
                      display_filters.OwnedFilter] = None) -> 'CardDisplays':
...
class CardDisplayPage:
    ...
    def run(self) -> pd.DataFrame:
        ...
        displays_on_page_df: pd.DataFrame \
    ...
    @staticmethod
    def _group_page(
            page: pd.DataFrame) -> pd.DataFrame:  # todo make less disgusting
        ...
        no_duplicates = original_index.drop_duplicates(
            subset=['set_num', 'card_num'])
    ...
    def format_ungrouped_page(page: pd.DataFrame) -> pd.DataFrame:
        ...
        page.drop_duplicates(subset=['set_num', 'card_num', 'count_in_deck'],
                             inplace=True)
...
def make_card_displays(user: models.user.User,
                       all_cards: models.card.AllCards) -> CardDisplays:
